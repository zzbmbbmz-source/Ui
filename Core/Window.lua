...
return function(MyUI, Theme, StatusColors, Utils, Services)
    ...
    -- ===== Window =====
    -- MyUI:CreateWindow({ Title, SubTitle, Size, Scale })
...
if Remote then
    ...
    oldFireServer = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if isRspyEnabled and method == "FireServer" and self:IsA("RemoteEvent") then
            pcall(function()
    ...
    oldInvokeServer = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if isRspyEnabled and method == "InvokeServer" and self:IsA("RemoteFunction") then
            pcall(function()
                local path = "game." .. self:GetFullName()
                local callingScript = "Unknown Script"
                local srcSuccess, srcObj = pcall(getcallingscript)
        ...
        Window:EnableInfoTab()
        Window:EnablePlayerTab()
        Window:EnableLoopTab()
        Window:EnableRemoteSpy()
